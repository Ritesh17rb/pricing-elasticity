...
def aggregate_active_base(brand_week, calendar, channel_rows, item_rows):
    weeks = [row["week_start"] for row in brand_week if row["brand_id"] == "pizzahut"]
    avg_orders = sum(num(row["system_orders"]) for row in brand_week if row["brand_id"] == "pizzahut") / max(1, len(weeks))

    channel_by_week_group = defaultdict(list)
    item_by_week_group = defaultdict(list)
    for row in channel_rows:
        if row["brand_id"] != "pizzahut":
            continue
    ...
    for index, week in enumerate(weeks):
        cal = calendar[week]
        brand = next(row for row in brand_week if row["week_start"] == week and row["brand_id"] == "pizzahut")
...
def build_events_and_windows(promo_rows):
    ...
    validation_windows = {"validation_windows": [
        ...
        {"window_id": "train_q2", "type": "train", "status": "clean", "start": "2025-06-02", "end": "2025-08-31", "weeks": 13, "purpose": "Summer and BOOK IT! carryout behavior."},
...
def build_scenarios():
    return [
        ...
        {"id": "scenario_baseline", "name": "Do Nothing (Baseline)", "category": "baseline", "model_type": "baseline", "description": "Maintain current Pizza Hut pricing and promotion settings with no change.", "impact_summary": "Baseline scenario for comparison", "config": {"tier": "all", "current_price": None, "new_price": None, "price_change_pct": 0, "promotion": None, "target_segment": "all", "effective_date": "2025-03-15", "grandfathering": False}, "constraints": {"platform_compliant": True, "price_change_12mo_limit": True, "notice_period_30d": True, "min_price": None, "max_price": None}, "priority": "n/a", "business_rationale": "Control scenario to measure incremental change."},
    ]


def build_metadata():
    return {
        "scope": "Pizza Hut active root data bundle",
        "generated_at": "2026-04-08",
        "datasets": {
            "customers": {"description": "Sample customer-level Pizza Hut records with channel mix and visit behavior.", "columns": {"customer_id": {"description": "Synthetic Pizza Hut customer identifier."}, "channel_group": {"description": "Internal group used by legacy models."}, "segment_key": {"description": "Composite 3-axis segment id."}}},
            "channel_weekly": {"description": "Weekly KPI panel by internal channel group, rebuilt for Pizza Hut.", "columns": {"week_start": {"description": "Week start date."}, "channel_group": {"description": "Internal compatibility grouping."}, "active_customers": {"description": "Proxy for active weekly order customers."}, "new_customers": {"description": "Modeled weekly new or reactivated guests."}, "repeat_loss_customers": {"description": "Modeled guests losing repeat frequency."}, "price": {"description": "Strategic list-price anchor used by scenario models."}, "effective_price": {"description": "List-price anchor adjusted for promo depth."}, "revenue": {"description": "Weekly net sales."}, "aov": {"description": "Average order value."}}},
            "season_calendar": {"description": "Demand windows and operational pressure indices for Pizza Hut.", "columns": {"week_start": {"description": "Week start date."}, "season_phase": {"description": "Modeled seasonal window."}, "demand_index": {"description": "Relative demand index vs average week."}}},
            ...
            "market_signals": {"description": "External market and competitive proxies aligned to Pizza Hut weeks.", "columns": {"week_start": {"description": "Week start date."}, "competitor_avg_price": {"description": "Modeled competitive market price."}, "macro_cpi": {"description": "Macro inflation proxy."}, "consumer_sentiment": {"description": "Consumer sentiment proxy."}}},
            "social_signals": {"description": "Modeled social attention and paid support aligned to Pizza Hut demand windows.", "columns": {"week_start": {"description": "Week start date."}, "total_social_mentions": {"description": "Modeled total social mentions."}, "social_sentiment": {"description": "Relative positive sentiment."}}},
            "retail_events": {"description": "Promo and competitive event log aligned to Pizza Hut pricing analysis.", "columns": {"event_id": {"description": "Event identifier."}, "week_start": {"description": "Week start date."}, "event_type": {"description": "Event classification."}, "promo_discount_pct": {"description": "Associated discount depth."}}},
            "segments": {"description": "Sample customer-to-segment assignments for Pizza Hut.", "columns": {"customer_id": {"description": "Sample customer identifier."}, "channel_group": {"description": "Internal compatibility grouping."}, "segment_key": {"description": "Composite segment id."}}},
...
def main():
    ...
    write_csv(DATA_DIR / "price_calendar.csv", price_calendar, list(price_calendar[0].keys()))
    write_csv(DATA_DIR / "market_signals.csv", market_signals, list(market_signals[0].keys()))
    write_csv(DATA_DIR / "social_signals.csv", social_signals, list(social_signals[0].keys()))
    write_csv(DATA_DIR / "season_calendar.csv", season_calendar, list(season_calendar[0].keys()))
    write_csv(DATA_DIR / "retail_events.csv", retail_events, list(retail_events[0].keys()))
    ...
    write_json(DATA_DIR / "cohort_coefficients.json", {"metadata": {"description": "Pizza Hut cohort profiles", "version": "6.0", "generation_date": "2026-04-08", "method": "Modeled Pizza Hut cohort calibration"}, **COHORT_PROFILES})
    ...
    write_json(DATA_DIR / "validation_windows.json", validation_windows)
    write_json(DATA_DIR / "metadata.json", build_metadata())
    print("Rebuilt Pizza Hut active data bundle.")
